...
<html ng-app="app">
	...
	<body>
		...
		<div id="root" class="container">
		    ...
		    <div id="top_nav" class="top-nav top_nav" style="display: none">
		      <div class="icon-back"></div>
		      <div class="nav-tittle">返回书架</div>
		    </div>
    		...
    		<div id="bottom-nav" class="bottom-nav bottom_nav" style="display: none">
		      <div class="bottom-container-1">
		          <div class="mulu-icon"></div>
		          <div class="mulu-font">目录</div>
		      </div>
		...
		<script>
			(function(){
				...
				var Util = (function(){
					...
					var getJsonp = function(url,callback){	//对jsonp格式数据发起的jsonp请求数据进行解码
						return $.jsonp({
							url:url,
							cache:true,
							callback:'duokan_fiction_chapter',
							success:function(result){	//jsonp请求成功后的回调函数，会拿到请求成功的结果集
								var data = $.base64.decode(result);		//对结果集进行base64格式解码
								var json = decodeURIComponent(escape(data));	//解析json获取能够显示的数据
								callback(json);
							}
						});
					}
				...
				var Chapter_id;		//声明章节id
				var ChapterTotal;	//声明总的章节数
				...
				function ReaderModel() {
					...
					var init = function(UIcallback){	//初始化数据层
					...
					var getFictionInfo = function(callback){	//获取章节概要信息
						 $.get('data/chapter.json',function(data){	
									//TODO 获得章节概要信息之后的回调方法，即get请求成功时，运行该方法
							 		Chapter_id = Util.StorageGetter("last_chapter_id");		//获取本地存储的章节id
							 		if (Chapter_id == null) {
							 			Chapter_id = data.chapters[1].chapter_id;	//如果本地没有存储的章节id，默认从1开始
									}
							 		ChapterTotal = data.chapters.length;
							 		callback && callback(data);		//先判断 callback 是不是存在，如果存在就执行
								},'json');
					}

					var getCurChapterContent = function(chapter_id,callback) {		//获取当前章节的内容
						$.get('data/data' + chapter_id + '.json',function(data){
								if(data.result == 0){	//提前约定好，result为0时代表服务器正常
									var url = data.jsonp;	//获取真实的指定章节的内容url，是一jsonp格式,发起一个jsonp请求
				...
				function ReaderBaseFrame(container) {
					//todo 渲染基本的UI结构
					function parseChapterData(jsonData){	//解析小说数据
				...
				function EventHanlder() {
					...
					$('.bottom_container_4').click(function() {		//由夜间模式切换为白天模式
						...
						$("body").css("background-color",Util.StorageGetter('background-color'));
						...
						Util.StorageSetter('night-day-background-color',Util.StorageGetter('background-color'));
					...
					$('.bk-container-1').click(function() {	//点击更换为白色背景
						...
						if (Util.StorageGetter("night-day-model") == "night") {
							Util.StorageSetter('night-day-model',"day");
							Dom.bottom_container_3.show();
							Dom.bottom_container_4.hide();
						}
					...
					$('#prev_button').click(function() {	//上一章点击翻页
						//TODO 获得章节的翻页数据，然后把数据拿出来渲染
						if (Chapter_id == 1) {
							return;
						}
						readerModel.prevChapter(function(data) {
							readerUI(data);
							location.href = "#fiction_container"; 
						});
						
					});

					$('#next_button').click(function() {	//下一章点击翻页
						if (Chapter_id == ChapterTotal ) {
							return;
						}
