...
public class SearchCDActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("SearchFragment6的search_type：",search_type+"");
        Log.d("SearchFragment6的search_resource：",search_resource+"");
        Log.d("SearchFragment6的用户序号：",user_id+"");

        //绑定搜索框控件
        sv2 = (SearchView)findViewById(R.id.sv2);

        //设置搜索框搜索提示
        sv2.setQueryHint("请输入光盘搜索内容");
        ...
        handler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == 52){    //显示光盘资源前三条历史搜索记录
                    //创建适配器
                    adapter = new SearchBookAdapter(SearchCDActivity.this,data);
                    //设置下拉列表项目点击监听事件
                    if (!("暂无任何历史记录".equals(adapter.data.get(1).getSearch_book_name()))){
                        Log.d("光盘第1次有历史记录测试！","光盘第1次有历史记录测试！");
                        //绑定适配器
                        ...
                        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                            ...
                            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                ...
                                if (position == 0){
                                    //应该跳转到我的全部光盘搜索历史记录界面
                                    Intent intent1 = new Intent(SearchCDActivity.this,UserAllSearchHistoryActivity.class);
                                    intent1.putExtra("user_id",user_id);
                                    intent1.putExtra("search_resource",search_resource);
                                    startActivity(intent1);
                                }else {
                                    //应该跳转到该条搜索历史记录搜索的光盘结果界面
                                    Intent intent1 = new Intent(SearchCDActivity.this,CDSearchResultActivity.class);
                                    intent1.putExtra("user_id",user_id);
                        ...
                        Log.d("光盘第1次无历史记录测试！","光盘第1次无历史记录测试！");
                    }
                    ...
                    sv2.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
                        ...
                        public boolean onQueryTextChange(String newText) {  //用户输入字符时触发该方法
                            if (TextUtils.isEmpty(newText.trim())){
                                if (!("暂无任何历史记录".equals(adapter.data.get(1).getSearch_book_name()))){
                                    Log.d("光盘第2次有历史记录测试！","光盘第2次有历史记录测试！");
                                    //绑定适配器
                                    lv.setAdapter(adapter);
                                    //设置下拉列表项目点击监听事件
                                    lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                    ...
                                    Log.d("光盘第2次无历史记录测试！","光盘第2次无历史记录测试！");
                                }
                                return true;
                            }
                            else {
            ...
            handler = new Handler(){
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (msg.what == 52){    //显示光盘资源前三条历史搜索记录
                        ...
                        //设置下拉列表项目点击监听事件
                        if (!("暂无任何历史记录".equals(adapter.data.get(1).getSearch_book_name()))){
                            Log.d("光盘第3次有历史记录测试！","光盘第3次有历史记录测试！");
                            //绑定适配器
                            lv.setAdapter(adapter);
                            lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                @Override
                            ...
                            Log.d("光盘第3次无历史记录测试！","光盘第3次无历史记录测试！");
                            lv.setAdapter(null);
                        }
                        ...
                        sv2.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
                            ...
                            public boolean onQueryTextChange(String newText) {  //用户输入字符时触发该方法
                                if (TextUtils.isEmpty(newText.trim())){
                                    if (!("暂无任何历史记录".equals(adapter.data.get(1).getSearch_book_name()))){
                                        Log.d("光盘第4次有历史记录测试！","光盘第4次有历史记录测试！");
                                        //绑定适配器
                                        lv.setAdapter(adapter);
                                        //设置下拉列表项目点击监听事件
                                        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                        ...
                                        Log.d("光盘第4次无历史记录测试！","光盘第4次无历史记录测试！");
                                        lv.setAdapter(null);
                                    }
                                    return true;
                                }
