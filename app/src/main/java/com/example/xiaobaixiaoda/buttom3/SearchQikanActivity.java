...
public class SearchQikanActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("SearchFragment2的search_type：",search_type+"");
        Log.d("SearchFragment2的search_resource：",search_resource+"");
        Log.d("SearchFragment2的用户序号：",user_id+"");

        //绑定搜索框控件
        sv2 = (SearchView)findViewById(R.id.sv2);

        //设置搜索框搜索提示
        sv2.setQueryHint("请输入期刊搜索内容");
        ...
        handler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == 39){    //显示前三条历史搜索记录
                    //创建适配器
                    adapter = new SearchBookAdapter(SearchQikanActivity.this,data);
                    //设置下拉列表项目点击监听事件
                    if (!("暂无任何历史记录".equals(adapter.data.get(1).getSearch_book_name()))){
                        ...
                        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                            ...
                            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                ...
                                if (position == 0){
                                    Toast.makeText(SearchQikanActivity.this, "应该跳转到我的全部期刊搜索历史记录界面", Toast.LENGTH_SHORT).show();
                                    Intent intent1 = new Intent(SearchQikanActivity.this,UserAllSearchHistoryActivity.class);
                                    intent1.putExtra("user_id",user_id);
                                    intent1.putExtra("search_resource",search_resource);
                                    startActivity(intent1);
                                }else {
                                    Toast.makeText(SearchQikanActivity.this, "你选择的期刊搜索记录是"+adapter.data.get(position).getSearch_book_name(), Toast.LENGTH_SHORT).show();
                                    Intent intent1 = new Intent(SearchQikanActivity.this,QiKanSearchResultActivity.class);
                                    intent1.putExtra("user_id",user_id);
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (msg.what == 39){    //显示前三条历史搜索记录
                        ...
                        sv2.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
                            @Override
                            public boolean onQueryTextSubmit(String query) {    //点击搜索按钮时触发该方法
                                Toast.makeText(SearchQikanActivity.this, "您输入的期刊查询内容是："+query, Toast.LENGTH_SHORT).show();
                                Intent intent1 = new Intent(SearchQikanActivity.this,QiKanSearchResultActivity.class);
                                intent1.putExtra("history_content",query);
                                intent1.putExtra("user_id",user_id);
                                intent1.putExtra("search_resource",search_resource);
